...
U256ID は、厳格かつ相互運用可能なエンコードおよび検証ルールを持つ 256-bit
識別子（uint256）です。

- 仕様: 下記の「U256ID — 使用仕様（規範）」を参照（MUST/SHOULD/MAY）。
- リポジトリ:
...
- コントラクトのビルド: `forge build`
- コントラクトのテスト: `forge test`
- SDK のテスト: `deno test packages/sdk`
